...
def generate_river(size, person_position, dperson_position):
    ...
    river[person_position[0]][person_position[1]] = "🏊‍♂️"
...
async def rescue(ctx,action:str):
    ...
    if action=="start":
        ...
        await ctx.send("You (🏊‍♂️) are trying to rescue a person (invisible) from drowning.")
...
async def go_into_field(ctx):
    with open('field_of_flowers.jpeg', 'rb') as f:
        ...
        await ctx.send(file=file)
        await ctx.send("You enter the field just for the plants to start moving and strangle you. You cut down the plants with your scythe🗡️ and manage to go and grab the neckalce. You wear the necklace📿 and it heals you. You continue walking in the 🌳forest🌳 until you hear the waterfall again.")
...
async def make_deal(ctx):
    ...
    await ctx.send("You accept the deal and the coin🪙. As you follow the map🗺️, you encounter a chamber.")
...
async def enter_chamber(ctx):
    with open('chamber.jpeg', 'rb') as f:
        ...
        await ctx.send("(Type `!enter_chamber` for the initial symbols. Eg. if the letters given are P A P E L  then to enter the correct answer type `!rearrange A P P L E`)")
...
async def rearrange(ctx, *symbols):
    ...
    if list(symbols) == CORRECT_PATTERN :
        await ctx.send("You've successfully rearranged the symbols. You step back as the rocks collapse in front of you. When everything settles, the weapon, a scythe lies at your feet. You pick it up and go from here. You continue following the map🗺️ and reach a field of tulip like flowers.")
        with open('field_of_flowers.jpeg', 'rb') as f:
            file = discord.File(f)
            await ctx.send(file=file)
            await ctx.send("As you go to cut the flower, in the middle of the field you can see a necklace📿 on a pedestal. A stone carving nearby proclaims that the necklace📿 heals its wearer of any life-threatening injuries. You start moving to the middle of the field just for the plants to start moving and strangle you. You cut down the plants with your scythe🗡️ and manage to go and grab the neckalce.")
