...
async def fight(ctx, action: str):
    ...
    if action == 'start':
        ...
        if user_hp >= 2:
            ...
            await ctx.send(f"Your HP is already at maximum. Monster HP: {monster_hp} , Your HP: {user_hp}")
            return

        user_hp += 1

        await ctx.send(f"You healed! Monster HP: {monster_hp}, Your HP: {user_hp}")
